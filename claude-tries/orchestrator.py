...
from llm_calls import run_prompt_1, run_prompt_2, run_prompt_3, run_prompt_6
...
def run_candidate(candidate: CandidateRun) -> CandidateRun:
    """Roda Prompt 3 -> execução -> repair loop para UM candidato."""
    ...
    while (not candidate.execution.passed) and candidate.repair_attempts < MAX_REPAIR_ATTEMPTS:
        ...
        repaired_statement = (
            f"{statement}\n\n[Código anterior falhou com:]\n{candidate.execution.stderr}\n"
            f"[Corrija o código sem enfraquecer a afirmação nem estreitar o range testado.]"
        )
        candidate.plan = run_prompt_3(repaired_statement, model=REPAIR_MODEL)
        candidate.execution = run_candidate_code(candidate.plan.code)
